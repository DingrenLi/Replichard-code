  ...
  public int run(String argv[]) throws Exception {
    ...
    try {
      if ("-put".equals(cmd) || "-copyFromLocal".equals(cmd)) {
        ...
        tail(argv, i);           
      } else if ("-testCreate1".equals(cmd)){
        String[] carg = Arrays.copyOfRange(argv, i, argv.length);
        testCreate2(carg);
      } else if ("-testDelete1".equals(cmd)){
        String[] carg = Arrays.copyOfRange(argv, i, argv.length);
        testDelete2(carg);
      } else if("-testOpen1".equals(cmd)){
        String[] carg = Arrays.copyOfRange(argv, i, argv.length);
        testOpen2(carg);
      } else {
        exitCode = -1;
...
private void testCreate2(String[] carg) {
        File dirPath = new File(carg[0]);
        int numOfOps = Integer.valueOf(carg[1]);
       try{
            fs.delete(new Path(dirPath.getPath()),true);
            fs.mkdirs(new Path(dirPath.getPath()));
            Thread.sleep(5000);
        } catch (IOException e) {
            e.printStackTrace();
         System.out.println(e.getCause() + " " + e.getMessage() + " " + dirPath.getPath() + "line 1884");
            return;
        } catch (InterruptedException e) {
            e.printStackTrace();
            System.out.println(e.getCause()+" "+e.getMessage()+" "+dirPath.getPath()+"line 1888");
            return;
        }
        byte[] buffer = new byte[0];

        long startTime = System.currentTimeMillis();
       System.out.println("start time is +"+dirPath+" "+startTime);
        for (int i = 1; i<=numOfOps; i++){
            File filePath = new File(dirPath, ""+i);
            try {
              FSDataOutputStream out = fs.create(new Path(filePath.getPath()), true, 1);
              out.write(buffer);
                out.close();
            } catch (IOException e) {
                System.out.println(e.getCause()+" "+e.getMessage()+" "+filePath.getPath()+"line 1903");
                e.printStackTrace();
            }
        }
   long endTime = System.currentTimeMillis();
   System.out.println("end time is +"+dirPath+" "+endTime);
}

  private void testDelete2(String[] carg) {
    File dirPath = new File(carg[0]);
    int numOfOps = Integer.valueOf(carg[1]);
    long startTime = System.currentTimeMillis();
    System.out.println("start time is +"+dirPath+" "+startTime);
    for (int i = 1; i<=numOfOps; i++){
      File filePath = new File(dirPath, ""+i);
      try {
        fs.delete(new Path(filePath.getPath()),true);
      } catch (IOException e) {
  ...
  private void testOpen2(String[] carg) {
    File dirPath = new File(carg[0]);
    int numOfOps = Integer.valueOf(carg[1]);
    byte[] buffer = new byte[0];
    long startTime = System.currentTimeMillis();
    ...
    for (int i = 1; i<=numOfOps; i++){
      File filePath = new File(dirPath, ""+i);
      try {
        FSDataInputStream in = fs.open(new Path(filePath.getPath()));
        in.read(buffer);
        in.close();
      } catch (IOException e) {
