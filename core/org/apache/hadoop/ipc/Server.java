...
public abstract class  Server {
  ...
  public ZMQ.Socket worker;
  public ZMQ.Socket cliPub;
  public String maprJobName="fisrt";

   public Subject userCopy=null;
   public ConnectionHeader headerCopy=new ConnectionHeader();
  protected int subscriberExpected;
  protected int numNamenode;
  ...
  int socketWriteTimeout = 0;                                                  // connections to nuke
                                                  //during a cleanup
 ...
 // public static void nnbind(ServerSocket socket,InetSocketAddress address,int backlog,){}
  ...
  private class Listener extends Thread {
    ...
    private InetSocketAddress address=null; //the address we bind at
    ...
    public Listener() throws IOException {
      ...
      //bind_MAN(ss,address,backlogLength);
      bind(ss, address, backlogLength);
   ...
   //public void bind_MAN(ServerSocket socket, InetSocketAddress address,
    //                     int backlog) throws IOException{
      //try{
       // bind(socket, address, backlog);
        //LOG.info("bind success");
        //port_count++;
     // }catch (BindException e){
       // LOG.info("port had already in use");
        //port=port+2;
       // port_count++;
       // address = new InetSocketAddress(bindAddress, port);
        //bind_MAN(socket,address,backlog);
     // }
    ...
    private void processHeader() throws IOException {
      ...
      try {
        ...
        if (protocolClassName != null) {
            LOG.info("header.getprotocal:"+header.getProtocol());
          protocol = getProtocolClass(header.getProtocol(), conf);
        }
        maprJobName=header.getMaprJobName();
  ...
  protected Server(String bindAddress, int port, 
    ...
    this.port_count=conf.getInt("id",0);
    this.numNamenode=conf.getInt("namenode.num",0);
    this.subscriberExpected=numNamenode-1;
  ...
  public synchronized void start_psub()throws IOException {
    context = ZMQ.context(1);
    subsocket = context.socket(ZMQ.SUB);
    subsocket.subscribe("".getBytes());
    /*subsocket.setHWM(0);
    subsocket.setLinger(-1);*/
     subsocket.setRcvHWM(0);
    subsocket.setLinger(-1);
    for (int i = 0; i < numNamenode; i++) {
      if (i == port_count) continue;
      subsocket.connect("tcp://" + conf.get("pub" + i + ".addr"));
      LOG.info("sub socket success connect to pub"+i);
    }


    if (port_count == 1) {
      sync = context.socket(ZMQ.REP);
      String addr = conf.get("rep.addr");
      sync.bind(addr);
      int subscrib = 0;
      while (subscrib < subscriberExpected) {
        byte[] result = sync.recv(0);
        sync.send("", 0);
        subscrib++;
        LOG.info("######rep result is:" + new String(result));
      }
    } else {
      sync = context.socket(ZMQ.REQ);
      sync.connect(conf.get("rep.addr"));
      sync.send(("hello from NN" + Integer.toHexString(port_count)).getBytes(), 0);
    }
    pubsocket = context.socket(ZMQ.PUB);
    pubsocket.bind("tcp://" + conf.get("pub" + Integer.toHexString(port_count) + ".addr"));
    pubsocket.setSndHWM(0);
    pubsocket.setLinger(-1);

    LOG.info("NN" + Integer.toHexString(port_count) + " socket created");
  }

    public synchronized  void startClipub(){
      ZMQ.Context con=ZMQ.context(1);
      cliPub = con.socket(ZMQ.PUB);
      cliPub.bind("tcp://" + conf.get("cli.pub"));
      cliPub.setSndHWM(0);
      cliPub.setLinger(-1);
    }
    public synchronized  void start_rdsock(ZContext ctx){
      client = ctx.createSocket(ZMQ.DEALER);
      String identity = String.format("%04X-%04X", rand.nextInt(), rand.nextInt());
      client.setIdentity(identity.getBytes());
      String cli_Front=conf.get("frontend.socket");
      client.connect(cli_Front);

      worker = ctx.createSocket(ZMQ.DEALER);
      String cli_Back=conf.get("backend.socket");
      worker.connect(cli_Back);
    ...
    /*if (port_count==0){
      ...
      pubsocket= context.socket(ZMQ.PUB);
      pubsocket.bind("tcp://"+conf.get("pub"+Integer.toHexString(port_count)+".addr"));
      pubsocket.setRcvHWM(0);
      pubsocket.setLinger(5000);
      ...
      sync = context.socket(ZMQ.REQ);
      sync.connect(conf.get("rep.addr"));
      sync.send("hello from NN1".getBytes(), 0);
      //NN2 SUB
      subsocket=context.socket(ZMQ.SUB);
      subsocket.setRcvHWM(0);
      for (int i=0;i<numNamenode;i++){
      ...
      sync = context.socket(ZMQ.REQ);
      sync.connect(conf.get("rep.addr"));
      sync.send("hello from NN2".getBytes(), 0);
