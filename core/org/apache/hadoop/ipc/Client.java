...
public class Client {
  ...
  public String maprJobName=null;
    ...
    public void run() {
      if (LOG.isDebugEnabled())
        LOG.debug(getName() + ": starting, having connections " 
            + connections.size());
     // LOG.info("server port in run:"+server.getPort());
  ...
  public Writable call(Writable param, InetSocketAddress addr, 
                       ...
                       throws InterruptedException, IOException {
   // LOG.info("address port in Client:"+addr.getPort());
    Call call = new Call(param);
    Connection connection = getConnection(addr, protocol, ticket, call);
    connection.sendParam(call);
  //LOG.info("protocal name is:"+protocol.getName());
    String protocalname="org.apache.hadoop.hdfs.protocol.ClientProtocol";
    ...
    /* public void connection_M(Writable param,InetSocketAddress addr,
                              Class<?> protocol, UserGroupInformation ticket)throws IOException, InterruptedException{
           value--;
           count++;
       String result=conf1.get("namenode"+Integer.toHexString(count));
       int colonIndex = result.indexOf(':');
       String address = result.substring(0, colonIndex);
       int port= Integer.valueOf(result.substring(colonIndex + 1));
      // LOG.info("namenode_M:"+address+result.substring(colonIndex+1));
       InetSocketAddress addr1=new InetSocketAddress(address, port);
       if (addr1!=addr){
         call(param,addr1, protocol, ticket);
       }}*/
