...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  public Block add_newblock_from_pub=new Block();
  public void set_pub_block(Block b){
    this.add_newblock_from_pub=b;
  }

  boolean first_namenode=false;
  public String gname[]=null;

  public String get_user(){return user;};
  public String[] get_gname(){return gname;}

  public void set_gname(String gname[]){this.gname=gname;}
  ...
  public String mNamenodeData=null;
  public String NamenodeData="";
  ...
  public Map locksFs=new HashMap();
  ...
  private void initialize(NameNode nn, Configuration conf) throws IOException {
    ...
    this.mNamenodeData=conf.get("default.namenode.datanode");
    this.NamenodeData=conf.get("fs.default.name");
    ...
    if(nameNodeAddress.getPort()==9000){
      first_namenode=true;
    }
  ...
  public LocatedBlock getAdditionalBlock(String src, 
    ...
    if(clientNode==null){
      DatanodeID  datanodeId=new DatanodeID("127.0.0.1:50010");
      DatanodeDescriptor clientnode=new DatanodeDescriptor(datanodeId);
...
//  if ((sub_task==false)&&(targets.length < this.minReplication)&&first_namenode) {
  ...
  private Block allocateBlock(String src, INode[] inodes) throws IOException {
    Block b=new Block();
    LOG.info("src is:"+src+" scrF is:"+ SrcFac.srcfac(src)+
            " in locksFs is:"+locksFs.get(SrcFac.srcfac(src))+" NamenodeData"+NamenodeData);
    if (locksFs.containsKey(SrcFac.srcfac(src))) {
      if (locksFs.get(SrcFac.srcfac(src)).equals(NamenodeData.replaceAll("hdfs://",""))){
      b = new Block(FSNamesystem.randBlockId.nextLong(), 0, 0);
  ...
  public void setLocks(Map locks){
     locksFs.putAll(locks);
  }
   ...
   */
    public boolean delete(String src, boolean recursive) throws IOException {
      ...
      String USER=UserGroupInformation.getCurrentUGI().getUserName();}
        ...
        user = UserGroupInformation.getCurrentUGI().getUserName();
        gname= UserGroupInformation.getCurrentUGI().getGroupNames();
  ...
  public boolean mkdirs(String src, PermissionStatus permissions
    ...
    if (status && auditLog.isInfoEnabled()) {
      final FileStatus stat = dir.getFileInfo(src);
      if(UserGroupInformation.getCurrentUGI()!=null){
      logAuditEvent(UserGroupInformation.getCurrentUGI(),
                    Server.getRemoteIp(),
  ...
  public synchronized void registerDatanode(DatanodeRegistration nodeReg
    ...
    if(!NamenodeData.equals(mNamenodeData)){
    dir.fsImage.setStorageInfo(nodeReg.storageInfo);}
    return;
  }
  ...
  private PermissionChecker checkPermission(String path, boolean doCheckOwner,
    ...
    String name=fsOwner.getUserName();
